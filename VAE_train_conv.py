...
from vox_encoder.vae_cnn import VAE_CNN, DecoderWrapperCNN_VAE, EncoderWrapperCNN_VAE
...
def train_model(
    ...
    for epoch in range(num_epochs):
        ...
        for inputs in train_loader:
            ...
            # Forward pass
            recon_x, mu, logvar = model(input)

            # Compute loss
            loss = vae_loss(recon_x, input, mu, logvar, weights, beta)
            loss.backward()
            optimizer.step()
...
def vae_loss(recon_x, x, mu, logvar, weights, beta=1.0):
    """
    Calculate the combined VAE loss: weighted BCE reconstruction + KL divergence.
    Parameters:
        recon_x: Reconstructed output from the decoder.
        x: Original input.
        mu: Mean of the latent distribution.
        logvar: Log-variance of the latent distribution.
        weights: Weighting factors for BCE loss.
        beta: Weighting factor for KL divergence.
    """
    # Weighted BCE Loss
    bce_loss = weighted_binary_cross_entropy(recon_x, x, weights=weights)
...
def save_model(model: nn.Module, optimizer, epoch, loss, latent_dim, input_size):
    ...
    # Save PyTorch model
    torch_path = Path(model_dir, "AE_checkpoint_conv_vae.pth")
    torch.save(
        {
    ...
    encoder = EncoderWrapperCNN_VAE(model.encoder_conv, model.fc_mu, model.fc_logvar)
    decoder = DecoderWrapperCNN_VAE(model.decoder_linear, model.decoder_conv, model.conv_dim, 64)

    # Save ONNX model
    encoder_path = Path(model_dir, f"encoder_conv_vae_opset{ONNX_OPSET}.onnx")
    decoder_path = Path(model_dir, f"decoder_conv_vae_opset{ONNX_OPSET}.onnx")
...
def main():
    ...
    model = VAE_CNN(latent_dim, input_dim)
